...
class StudyCreateServiceTest {
    ...
    void 스터디생성_실패_유저존재하지않음() {
        ...
        // when
        UserNotFoundException exception = assertThrows(UserNotFoundException.class, () -> studyCreateService.createNewStudy(request));
    ...
    void 스터디생성_성공() {
        ...
        doReturn(new StudyPerUser(getUser(), getStudy())).when(studyPerUserRepository).save(any(StudyPerUser.class));

        // when
        Study newStudy = studyCreateService.createNewStudy(getRegisterStudyRequest());
